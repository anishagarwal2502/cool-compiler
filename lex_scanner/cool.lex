...
<YYINITIAL> "(*"		{ yybegin(COMMENT); curr_comment_count++; }
...
<YYINITIAL> "\""		{ yybegin(STRING); string_buf.delete(0, string_buf.length()); }
<YYINITIAL> "--"		{yybegin(COMMENT); in_dash_comment = true; }
...
<STRING> \\[btnf\"\\]		{ char c = yytext().charAt(1);
                                  switch(c){
...
<STRING> {NULL}			{ null_string = true;
				 return new Symbol(TokenConstants.ERROR, "String contains null character"); }

<STRING> {ALPHANUM}+|{WHITESPACE}+|{GOOD}|{BAD}	{ string_buf.append(yytext()); }

<STRING> \\{NEWLINE_CHAR}	{ curr_lineno++;
...
<STRING> {NEWLINE_CHAR}		{ yybegin(YYINITIAL);
...
<COMMENT> {NEWLINE_CHAR}	{ curr_lineno++;
				  if(in_dash_comment){
					in_dash_comment = false;
					yybegin(YYINITIAL);
				  }
